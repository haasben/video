...
function is_mobile($text){
    ...
    if(preg_match("/^1[345678]{1}\d{9}$/",$text)){
...
function send_msg_api($mobile,$code){

    $url = 'http://api.1cloudsp.com/api/v2/single_send?';
    $data = [
        'accesskey'=>'4PVPoonI7gB8OQPQ',
        'secret'=>'eRbk23yiaggInrMWpZwbnFhqg8Ex0VC4',
        'sign'=>'139099',
        'templateId'=>'167014',
        'mobile'=>$mobile,
        'content'=>"{$code}##3",
    ...
    $return_msg = file_get_contents($url.http_build_query($data));
    return json_decode($return_msg,true);
