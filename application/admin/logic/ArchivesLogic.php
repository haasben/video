...
class ArchivesLogic extends Model
{
     ...
     * 删除文档
     */
    public function course_del($del_id = array(), $thorough = 0)
    {
        if (empty($del_id)) {
        ...
        if(!empty($id_arr)){
            ...
            if (1 == $thorough) { // 直接删除，跳过回收站
                ...
                foreach ($data as $key => $val) {
                    $r = M('course')->where('aid','IN',$val['aid'])->delete();
                    if ($r) {
                        model($val['ctl_name'])->course_afterDel($val['aid']);
                ...
                foreach ($data as $key => $val) {
                    ...
                    $r = M('course')->where('aid','IN',$val['aid'])->update($info);
                    if ($r) {
