...
class Ueditor extends Base
{
    ...
    */
    ...
    public function DownloadUploadFile(){
        ...
        // dump(tpCache('basic.file_size'));die;
        // 若获取不到则定义为空
        ...
        //上传文件
    public function NewDownloadUploadFile(){
        

        $file_name = $_POST['new_file_name'];
        $old_file_name = $_POST['file_name'];
        $file_ext = strrchr($old_file_name,'.');
        ...
        $res = $obj->upload([
            ...
            'file_name' => $file_name, // 文件名称(唯一)
            'file_path' => '/uploads/video/'.date('Ymd'), // 文件存放路径
        ...
        if($res['code'] == 1){
            // 拼装数据存入session
            $file_path = $res['url'];
            cache($_POST['file_name'],'');
            $return = array(
                'code'      => 1,
                'msg'       => '上传成功',
                'file_url'  => $res['url'],
                'file_mime' => $_POST['type'],
                'file_name' => $file_name,
                'file_ext'  => $file_ext,
                'file_size' => round(filesize(ROOT_PATH.$file_path)/1024),
                'uhash'     => $this->uhash(ROOT_PATH.$file_path),
                'md5file'   => md5_file(ROOT_PATH.$file_path),
            );
