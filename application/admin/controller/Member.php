...
class Member extends Base {
    ...
    public function users_batch_add()
    {
        if (IS_POST) {
            ...
            $post['view_rights'] = implode(',', $post['view_rights']);
            if($post['view_rights'] == NULL){
                $post['view_rights'] = 1;
            }
    ...
    public function users_edit()
    {
        ...
        $info['view_rights'] = explode(',', $info['view_rights']);
    ...
    private function check_alipay_order($order_number,$admin_pay='',$alipay='')
    {
        if (!empty($order_number)) {
            ...
            $result = json_decode(json_encode($result),true);
             Db::name('linshi')->insert(['info'=>json_encode($result),'time'=>date('Y-m-d H:i:s')]);
            // 判断结果
