...
class Download extends Model
{
    ...
    public function afterSave($aid, $post, $opt)
    {
        ...
        public function afterSaveCourse($aid, $post, $opt)
    {
        $post['aid'] = $aid;
        $addonFieldExt = !empty($post['addonFieldExt']) ? $post['addonFieldExt'] : array();
       // model('Field')->dealChannelPostData($post['channel'], $post, $addonFieldExt);
     ...
     * @author wengxianhu by 2017-7-26
     */
    public function getCourseInfo($aid, $field = '', $isshowbody = true)
    {
        $result = array();
        $field = !empty($field) ? $field : '*';
        $result = db('course')->field($field)
            ->where([
                'aid'   => $aid,
                'lang'  => get_admin_lang(),
            ])
     ...
     * @param int $aid
     */
    public function course_afterDel($aidArr = array())
    {
        if (is_string($aidArr)) {
