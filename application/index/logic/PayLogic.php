...
class PayLogic extends Model
{
    ...
    public function wechat_return($data){


        $param = $data;
        $return_attach = explode('_', $param['attach']); 
        $transaction_type = $return_attach['1'];
        

        Db::name('linshi')->insert(['info'=>json_encode($transaction_type),'time'=>'4_'.date('Y-m-d H:i:s')]);
        // 实际付款金额
        $order_amount = $data['total_fee']/100;
        if ('2' == $transaction_type) {
            // 商城订单购买支付回调处理
            $return = $this->wechatShopOrderProcessing($param, $order_amount);
            return $return;

        }else if ('1' == $transaction_type) {

            $level = $return_attach['2'];
            // 会员充值或升级支付回调处理
            $return = $this->wechatMoneyOrderProcessing($param, $order_amount,$level);
            return $return;
        }
    ...
    // 商城订单购买支付回调处理
    private function wechatShopOrderProcessing($param = array(), $order_amount = null)
    {


        if (!empty($param['out_trade_no']) && !empty($param['transaction_id'])) {
            $OrderWhere = [
                'lang'       => $this->home_lang,
                // 订单号
           ...
           // Db::name('linshi')->insert(['info'=>json_encode($OrderData),'time'=>date('Y-m-d H:i:s',time())]);
            if (!empty($OrderData)) {
                ...
                if (0 == $OrderData['order_status']) {
                    ...
                    $UpOrderData = [
                        'order_status' => 1,
                        'pay_name'     => 'wechat', // 支付宝支付
                        'wechat_pay_type' => 'wechat_native',    // 清空微信标记
                        'pay_details'  => serialize($param),
                        'pay_time'     => $time,
                        'update_time'  => $time,
                    ];
                    $ReturnId = $this->shop_order_db->where($OrderWhere)->update($UpOrderData);
                     Db::name('linshi')->insert(['info'=>json_encode($ReturnId),'time'=>date('Y-m-d H:i:s',time())]);
                    if (!empty($ReturnId)) {
                        // 更新商城订单副表
                        $DetailsData['update_time'] = $time;
                        $this->shop_order_details_db->where('order_id',$OrderData['order_id'])->update($DetailsData);
                        // 添加订单操作记录
                        AddOrderAction($OrderData['order_id'],$OrderData['users_id'],'0','1','0','1','支付成功！','会员使用微信完成支付！');
    ...
    // 会员充值或升级支付回调处理
    private function wechatMoneyOrderProcessing($param = array(), $order_amount = null,$level)
    {
        if (!empty($param['out_trade_no']) && !empty($param['transaction_id'])) {
            // 付款成功
            $MoneyWhere = [
            ...
            // 支付宝订单统一处理
            $msg = $this->wechatMoneyUnifiedProcessing($param, $MoneyData, $order_amount,$level);
            return $msg;
        }
    ...
    // 参数2为充值记录表数据集
    // 参数3为订单实际付款金额
    private function wechatMoneyUnifiedProcessing($param, $MoneyData, $PayMoney,$level){
        // 支付宝付款成功后，订单并未修改状态时，修改订单状态并返回
        if ($MoneyData['status'] == 1) {
            ...
            if (!empty($ReturnId)) {
                ...
                if($level == 5){
                   $view_rights = '1,2,3,4,5'; 
                }else{
                    $view_rights = DB::name('users')->where('users_id',$MoneyData['users_id'])->limit(1)->value('view_rights');
                    $view_rights = trim($view_rights.','.$level);
                }
                
                $is_activation = DB::name('users')->where('users_id',$MoneyData['users_id'])->update([
                    'is_activation'=>1,
                    'level'=>$level,
                    'view_rights'=>$view_rights
            ]);
  
                cookie('user_money',NULL);
                if (!empty($is_activation)) {
    ...
    public function alipay_return()
    {
        ...
        Db::name('linshi')->insert(['info'=>json_encode($param),'time'=>'1_'.date('Y-m-d H:i:s',time())]);
        // 支付宝配置信息
        ...
        Db::name('linshi')->insert(['info'=>json_encode($param),'time'=>'2_'.date('Y-m-d H:i:s',time())]);
    ...
    private function MoneyOrderProcessing($param = array(), $order_amount = null)
    {
        ...
        Db::name('linshi')->insert(['info'=>json_encode($param),'time'=>'3_'.date('Y-m-d H:i:s',time())]);
        if (!empty($param['out_trade_no']) && !empty($param['trade_no'])) {
            // 付款成功
            $MoneyWhere = [
                'lang'         => $this->home_lang,
    ...
    private function MoneyUnifiedProcessing($param, $MoneyData, $PayMoney){
        ...
        if ($MoneyData['status'] == 1) {
            ...
            $ReturnId = $this->users_money_db->where($where)->update($UpMoneyData);
            if (!empty($ReturnId)) {
                Db::name('linshi')->insert(['info'=>json_encode($param),'time'=>'4_'.date('Y-m-d H:i:s',time())]);
                if($param['level'] == 5){
                   $view_rights = '1,2,3,4,5'; 
                }else{
                    $view_rights = DB::name('users')->where('users_id',$MoneyData['users_id'])->limit(1)->value('view_rights');
                    $view_rights = trim($view_rights.','.$param['level']);
                }
