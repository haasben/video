...
$(function () {
    ...
    var courses = {
        ...
        loginDialog: function () {
            $.fancybox.open({
                href: 'http://www.jcpeixun.com/login/logins.html?callback=' + escape(location.href),
                type: 'iframe',
                padding: 10,
                width: '400',
                height: '440',
        ...
        */
        ...
        Checkmobilepass: function () {
            ...
            if (login == "1" && mobile == "0") {
                $.Boxen.open(window, {
                    url: "http://www.jcpeixun.com/emailregister/v_mobile.aspx",
        ...
        _download: function (lessname, _lessonid) {
          //  if (courses.CheckLogin(1)) {
            var downpage = "http://course.jcpeixun.com/p/down_new.aspx";
    ...
    if ($lesson_20140110.length) {
        ...
        $("[id^='addcart']").click(function (event) {
            ...
            var _domainPrefix = "http://www.jcpeixun.com/";

            //            if (courses.CheckLogin(1)) {
            $.getJSON(_domainPrefix + "ashx/order/cart2.aspx?jsoncallback=?", { action: "add", lessonid: lessonid }, function (data) {
                $.Boxen.open(window, {
                    url: "/pay/cartaddSuccess",
                    urlParams: { callback: document.URL, t: escape(data.msg) },
                    width: 420, height: 260
                });
            });
            // };
        ...
        function cislogin(away) {
            ...
            if (login == null) {
                if (away == "0" || away == "") {
                    ...
                    $.Boxen.open(window, {
                        url: "http://www.jcpeixun.com/windows/login_register.aspx",
